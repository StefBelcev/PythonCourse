...
def divide_classes(dataset):
    zero_class = [dataset[i] for i in range(len(dataset)) if dataset[i][-1] == 0]
    one_class = [dataset[i] for i in range(len(dataset)) if dataset[i][-1] == 1]
    two_class = [dataset[i] for i in range(len(dataset)) if dataset[i][-1] == 2]
    return zero_class, one_class, two_class


def divide_sets_NB(zero_class, one_class, two_class, x1, x2):
    train_set = zero_class[:int(x1 * len(zero_class))] + one_class[:int(x1 * len(one_class))] + \
                two_class[:int(x1 * len(two_class))]
    test_set = zero_class[int(x2 * len(zero_class)):] + one_class[int(x2 * len(one_class)):] + \
               two_class[int(x2 * len(two_class)):]
    return train_set, test_set


def divide_sets_DT(zero_class, one_class, two_class, x1, x2):
    train_set = zero_class[int(x1 * len(zero_class)):int(x2 * len(zero_class))] + \
                one_class[int(x1 * len(one_class)):int(x2 * len(one_class))] + \
                two_class[int(x1 * len(two_class)):int(x2 * len(two_class))]
    test_set = zero_class[int(x2 * len(zero_class)):] + one_class[int(x2 * len(one_class)):] + two_class[int(x2 * len(
        two_class)):]
    return train_set, test_set


def divide_sets_RF(zero_class, one_class, two_class, x2):
    train_set = zero_class[:int(x2 * len(zero_class))] + one_class[:int(x2 * len(one_class))] + two_class[:int(
        x2 * len(two_class))]
    test_set = zero_class[int(x2 * len(zero_class)):] + one_class[int(x2 * len(one_class)):] + two_class[int(x2 * len(
        two_class)):]
    return train_set, test_set
...
def divide_sets_MLP(zero_class, one_class, two_class, x1, x2):
    train_set = zero_class[:int(x1 * len(zero_class))] + one_class[:int(x1 * len(one_class))] + two_class[:int(x1 * len(two_class))]
    val_set = zero_class[int(x1 * len(zero_class)):int(x2 * len(zero_class))] + one_class[int(x1 * len(one_class)):int(x2 * len(one_class))] + two_class[int(x1 * len(two_class)):int(x2 * len(two_class))]
    test_set = zero_class[int(x2 * len(zero_class)):] + one_class[int(x2 * len(one_class)):] + two_class[int(x2 * len(
        two_class)):]
    return train_set, val_set, test_set


def divide_characteristics(given_set):
    x_values = [given_set[i][:-1] for i in range(len(given_set))]
    y_values = [given_set[i][-1] for i in range(len(given_set))]
...
if __name__ == '__main__':
    ...
    # dividing datasets
    zero_dataset, one_dataset, two_dataset = divide_classes(dataset)

    # creating training and testing sets for each classificator
    # NAIVE BAYES
    nb_train_set, nb_test_set = divide_sets_NB(zero_dataset, one_dataset, two_dataset, x1, x2)
    # DECISION TREE
    dt_train_set, dt_test_set = divide_sets_DT(zero_dataset, one_dataset, two_dataset, x1, x2)
    # RANDOM FOREST TREE
    rf_train_set, rf_test_set = divide_sets_RF(zero_dataset, one_dataset, two_dataset, x2)
    # NEURAL NETWORK
    mlp_train_set, mlp_val_set, mlp_test_set = divide_sets_MLP(zero_dataset, one_dataset, two_dataset, x1, x2)

    # dividing X and Y values for the training sets for each classificator
    # NAIVE BAYES
    nb_train_set_x, nb_train_set_y = divide_characteristics(nb_train_set)
    # DECISION TREE
    dt_train_set_x, dt_train_set_y = divide_characteristics(dt_train_set)
    # RANDOM FOREST TREE
    rf_train_set_x, rf_train_set_y = divide_characteristics(rf_train_set)
    # NEURAL NETWORK
    mlp_train_set_x, mlp_train_set_y = divide_characteristics(mlp_train_set)


    # creating and training for each classificator
    # NAIVE BAYES
    nb = GaussianNB()
    nb.fit(nb_train_set_x, nb_train_set_y)
    # DECISION TREE
    dt = DecisionTreeClassifier(criterion='entropy')
    dt.fit(dt_train_set_x, dt_train_set_y)
    # RANDOM FOREST TREE
    rf = RandomForestClassifier(n_estimators=3, criterion='entropy')
    rf.fit(rf_train_set_x, rf_train_set_y)
    # NEURAL NETWORK
    mlp = MLPClassifier(5, activation='relu', learning_rate_init=0.001, max_iter=500, random_state=0)
    mlp.fit(mlp_train_set_x, mlp_train_set_y)

    # dividing X and Y values for the test sets for each classificator
    # NAIVE BAYES
    nb_test_set_x, nb_test_set_y = divide_characteristics(nb_test_set)
    # DECISION TREE
    dt_test_set_x, dt_test_set_y = divide_characteristics(dt_test_set)
    # RANDOM FOREST TREE
    rf_test_set_x, rf_test_set_y = divide_characteristics(rf_test_set)
    # NEURAL NETWORK
    mlp_test_set_x, mlp_test_set_y = divide_characteristics(mlp_test_set)

    # accuracy of the model, if at least 2 classificators are predicting the same class
    accuracy = 0
    for nb_x, nb_y, dt_x, dt_y, rf_x, rf_y in zip(nb_test_set_x, nb_test_set_y, dt_test_set_x, dt_test_set_y, rf_test_set_x, rf_test_set_y):
        nb_predicted = nb.predict([nb_x])[0]
        dt_predicted = dt.predict([dt_x])[0]
        rf_predicted = rf.predict([rf_x])[0]
        if nb_predicted == nb_y and dt_predicted == dt_y or rf_predicted == rf_y and dt_predicted == dt_y or nb_predicted == nb_y and rf_predicted == rf_y:
            accuracy += 1
        else:
            accuracy += 0

    print(f'Tochnost: {accuracy/len(nb_test_set)}')
